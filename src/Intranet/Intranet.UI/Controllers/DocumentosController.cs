...
namespace Intranet.UI.Controllers
{
    public class DocumentosController : BaseController
    {
        ...
        public ActionResult Lista(TipoDocumento tipo, string busca, int? idEmpreendimento, Ordenacao? ordenacao, int? idCategoria)
        {
            ...
            {               
                documentos = documentos.Where(c => (Usuario.Empreendimento.Id == 10 ? true : (c.IdEmpreendimento.ToString().Contains(Usuario.Empreendimento.Id.ToString()) || c.IdEmpreendimento.ToString() == "10")));
            }
