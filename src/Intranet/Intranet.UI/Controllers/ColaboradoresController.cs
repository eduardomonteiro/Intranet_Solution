...
namespace Intranet.UI.Controllers
{
    public class ColaboradoresController : BaseController
    {
        ...
        public ActionResult Lista(string busca, Ordenacao? ordenacao)
        {
            ...
            if (User.IsInRole("Colaboradores-Localidade"))
            {
                usuarios = usuarios.Where(c => (Usuario.Empreendimento.Id == 10 ? true : (c.IdEmpreendimento.ToString().Contains(Usuario.Empreendimento.Id.ToString()) || c.IdEmpreendimento.ToString() == "10")));
            }
            ...
            if (ordenacao != null)
            {
                ...
                if (ordenacao == Ordenacao.Empreendimento)
                {
                    usuarios = usuarios.OrderBy(x => x.Empreendimento.Nome);
                }
            }
            else
            {
