...
namespace $safeprojectname$.Controllers
{
    ...
    public class LoginController : Controller
    {
        ...
        [HttpPost]
        [AllowAnonymous]
        public async Task<ActionResult> Index(LoginVm model, string returnUrl)
        {
            ...
            var result = await SignInManager.PasswordSignInAsync(model.Email, model.Password, model.LembrarMe, shouldLockout: false);
            switch (result)
            {
        ...
        public void ForgotPassword(ForgotPasswordViewModel model)
        {
            if (ModelState.IsValid)
            {
                ...
                if (user != null)
                {
                    ...
                    var conteudoEmail = System.IO.File.ReadAllText(Server.MapPath("/Views/Login/mailSenha.html"));
                    conteudoEmail = conteudoEmail.Replace("##link##", callbackUrl);
                    Email.EnviaEmail("informatica@partage.com.br", model.Email, "Recuperação de Senha", conteudoEmail);
                }
            }
        }
